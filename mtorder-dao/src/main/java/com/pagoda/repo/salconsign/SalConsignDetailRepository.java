...
public interface SalConsignDetailRepository
  ...
  @SqlTemplate(
    ...
    sql =
        "select con_seqno,con_id,goods_id,order_seqno,goods_code,goods_name,goods_spec,sal_unit_name,net_weight, shipped_net_weight,qty,shipped_qty,tax_rate,tax_amt,price,total_amt,discount_rate,discount_price, actual_amt,shipped_price,shipped_count,shipped_amt,remark,shipped_gross_weight,gross_weight, version from sal_consign_detail where deleted = 0 {{#con_id}} and con_id=:con_id {{/con_id}} {{#creator_org_code}} and creator_org_code in ({{#creator_org_code}}{{^-first}}, {{/-first}}\"{{this}}\"{{/creator_org_code}}) {{/creator_org_code}} "
  )
  ...
  List<SalConsignDetailDTO> findSalConsignDetail(
      @Param("con_id") Long conId, @Param("creator_org_code") String creatorOrgCode);
